<template>
  <div id="big_back">
          ...
          <ul class="list-group">
            <li class="list-group-item d-flex" id="head_li">
              <div class="table_top">
                ...
                <strong>房间类型</strong>
                <strong>入住人数</strong>
                <strong>最大入住人数</strong>
                <strong class="op">操作</strong>
              </div>
            </li>
            <!-- 通过 v-for 渲染所有的活动事件记录 -->
            <li v-for="room in rooms" :key="room.id" class="list-group-item d-flex">
              <div class="table_top">
                <!-- 渲染活动事件的具体数据 -->
                <div>{{ room.id }}</div>
                <div>{{ room.charge.item }}</div>
                <div>{{ room.intoNumber }}</div>
                <div>{{ room.maxNumber }}</div>

                <span class="op_btn">
                  <!-- 操作按钮，点击分别调用详情、修改、删除方法 -->
                  <a class="btn btn_style_lo btn-sm" @click="detail(room.id)">详情</a>
                  <a class="btn btn_style_mo btn-sm" @click="modify(room.id)">修改</a>
                  <a class="btn btn_style_del btn-sm" @click="remove(room.id)">删除</a>
                </span>
              </div>
            </li>
          </ul>
    ...
    <!-- 新增表单 -->
    <el-dialog v-model="addFormVisible" title="新增房间" width="50%">
      <el-form :model="addForm" label-width="120px">
        <el-form-item label="房间ID">
          <el-input v-model="addForm.id"></el-input>
        </el-form-item>
        <el-form-item label="房间类型ID">
          <el-select v-model="addForm.typeId" class="top_select">
            <el-option :label="'单人间'" :value="5"></el-option>
            <el-option :label="'双人间'" :value="6"></el-option>
            <el-option :label="'三人间'" :value="7"></el-option>
            <el-option :label="'五人间'" :value="8"></el-option>
          </el-select>

        </el-form-item>
        <el-form-item label="入住人数">
          <el-input v-model="addForm.intoNumber"></el-input>
        </el-form-item>
    ...
    <!-- 详情表单表单 -->
    <el-dialog v-model="detailFormVisible" title="房间详情" width="50%">
      <el-form :model="detailForm" label-width="120px">
        ...
        <el-form-item label="房间类型">
          <el-input v-model="detailForm.charge.item" disabled></el-input>
        </el-form-item>
        <el-form-item label="入住人数">
          <el-input v-model="detailForm.intoNumber" disabled></el-input>
        </el-form-item>
        <el-form-item label="最大入住人数">
          <el-input v-model="detailForm.maxNumber" disabled></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <!-- 更新表单 -->
    <el-dialog v-model="modifyFormVisible" title="修改房间" width="50%">
      <el-form :model="modifyForm" label-width="120px">
        ...
        <el-form-item label="房间类型ID">
          <el-input v-model="modifyForm.typeId" ></el-input>
        </el-form-item>
        <el-form-item label="入住人数">
          <el-input v-model="modifyForm.intoNumber" ></el-input>
        </el-form-item>
        <el-form-item label="最大入住人数">
          <el-input v-model="modifyForm.maxNumber" disabled ></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
...
import { addRoom, deleteRoom, modifyRoom, showList4 } from "@/api/other/room";
...
// 监听 addForm.typeId 的变化
watch(() => addForm.value.typeId, (newVal) => {
  if (newVal !== '') {
    addForm.value.maxNumber = addForm.value.maxNumberMap[newVal];
  } else {
    addForm.value.maxNumber = '';
  }
});
