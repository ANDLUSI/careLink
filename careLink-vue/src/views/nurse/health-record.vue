<template>
  <div id="big_back">
      ...
      <div class="main-page">
        <div class="grids widget-shadow">
          ...
          <ul class="list-group">
            ...
            <li v-for="incident in incidents" :key="incident.id" class="list-group-item d-flex">
              <div class="table_top">
                <!-- 渲染突发事件的具体数据 -->
                <div>{{ incident.residentId }}</div>
                <div>{{ incident.incidentType }}</div>
                <div>{{ incident.occurrenceTime }}</div>
                <div>{{ incident.description }}</div>
                <div>{{ incident.resolution }}</div>
                <div>{{ incident.handlerId }}</div>
                <span class="op_btn">
                  <!-- 操作按钮，点击分别调用详情、修改、删除方法 -->
                  <a class="btn btn_style_lo btn-sm" @click="detail(incident.id)">详情</a>
                  <a class="btn btn_style_mo btn-sm" @click="modify(incident.id)">修改</a>
                  <a class="btn btn_style_del btn-sm" @click="remove(incident.id)">删除</a>
                </span>
              </div>
            </li>
          </ul>
    ...
    <!-- 新增表单 -->
    <el-dialog v-model="addFormVisible" title="新增突发事件记录" width="50%">
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="120px">
        ...
        <el-form-item label="事件名称">
          <el-input v-model="addForm.incidentType"></el-input>
        </el-form-item>
        <el-form-item label="发生时间">
          <el-date-picker v-model="addForm.occurrenceTime" type="datetime" placeholder="选择日期时间"></el-date-picker>
        </el-form-item>
        <el-form-item label="详细描述">
          <el-input v-model="addForm.description" type="textarea"></el-input>
        </el-form-item>
        <el-form-item label="处理结果">
          <el-input v-model="addForm.resolution"></el-input>
        </el-form-item>
        <el-form-item label="处理人">
          <el-input v-model="addForm.handlerId"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 详情表单表单 -->
    <el-dialog v-model="detailFormVisible" title="突发事件详情" width="50%">
      <el-form :model="detailForm" label-width="120px">
        <el-form-item label="入住人id">
          <el-input v-model="detailForm.residentId" disabled></el-input>
        </el-form-item>
        <el-form-item label="事件名称">
          <el-input v-model="detailForm.incidentType" disabled></el-input>
        </el-form-item>
        <el-form-item label="发生时间">
          <el-date-picker v-model="detailForm.occurrenceTime" type="datetime" placeholder="选择日期时间" disabled></el-date-picker>
        </el-form-item>
        <el-form-item label="详细描述">
          <el-input v-model="detailForm.description" type="textarea"></el-input>
        </el-form-item>
        <el-form-item label="处理结果">
          <el-input v-model="detailForm.resolution" disabled></el-input>
        </el-form-item>
        <el-form-item label="处理人">
          <el-input v-model="detailForm.handlerId" disabled></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <!-- 更新表单 -->
    <el-dialog v-model="modifyFormVisible" title="修改突发事件" width="50%">
      <el-form :model="modifyForm" label-width="120px">
        ...
        <el-form-item label="事件名称">
          <el-input v-model="modifyForm.incidentType" ></el-input>
        </el-form-item>
        <el-form-item label="发生时间">
          <el-date-picker v-model="modifyForm.occurrenceTime" type="datetime" placeholder="选择日期时间" ></el-date-picker>
        </el-form-item>
        <el-form-item label="详细描述">
          <el-input v-model="modifyForm.description" type="textarea"></el-input>
        </el-form-item>
        <el-form-item label="处理结果">
          <el-input v-model="modifyForm.resolution" ></el-input>
        </el-form-item>
        <el-form-item label="处理人">
          <el-input v-model="modifyForm.handlerId" ></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
...
const addFormRules = {
  residentId: [
    { required: true, message: '请输入入住人 ID', trigger: 'blur' },
    { type: 'number', message: '入住人 ID 必须是数字', trigger: 'blur' },
  ],
  incidentType: [
    { required: true, message: '请输入事件名称', trigger: 'blur' },
    { max: 50, message: '事件名称长度不能超过 50 字符', trigger: 'blur' },
  ],
  occurrenceTime: [
  ...
  description: [
    ...
    { max: 500, message: '详细描述不能超过 500 字符', trigger: 'blur' },
  ],
  resolution: [
    { required: true, message: '请输入处理结果', trigger: 'blur' },
  ],
  handlerId: [
    { required: true, message: '请输入处理人 ID', trigger: 'blur' },
    { max: 50, message: '处理人 ID 不能超过 50 字符', trigger: 'blur' },
  ],
};
