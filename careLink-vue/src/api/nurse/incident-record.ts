...
export const showList = async ({key = '', pageNum = 1, pageSize = 10}: FetchIncidentsParams) => {
  try {
    const response = await axios.get('http://localhost/IncidentRecordController/list', {
      params: {
        key,
        pageNum,
        pageSize
...
export const addIncident = async (data: {
  ...
  try {
    ...
    const response = await axios({
      url: 'http://localhost/IncidentRecordController/save',
      method: 'post',
      data: data,
      headers: {
        Token:sessionStorage.getItem("token")
...
//修改方法
export const modifyIncident = async (data: {
  id: string;
  residentId: string;
  incidentType: string;
  occurrenceTime: string;
  ...
  try {
    console.log(data);
    const response = await axios({
      url: 'http://localhost/IncidentRecordController/update',
      method: 'post',
      data: data,
      headers: {
        Token:sessionStorage.getItem("token")
...
export const deleteIncident = async (id) => {
  try {
    console.log(id);
    const response = await axios({
      url: 'http://localhost/IncidentRecordController/remove',
      method: 'get',
      params: {
        id: id
      },
